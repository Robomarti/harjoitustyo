...
class TestCollisionDetector(unittest.TestCase):
    ...
    def test_handle_player_death(self):
        ...
        logic.collision_detector.handle_player_death(self.game_map, self.player, self.projectile, "p1")
        self.assertEqual(self.game_map[1], ["x", " ", " ", "x"])

    def test_flag_drops_after_player_death(self):
        player2 = sprites.player.Player([], 2, [1,1], [0,0])
        player2.has_flag = True
        game_map = [["x", "p2", " ", "x"], ["x", " ", " ", "x"]]
        self.assertEqual(game_map[1], ["x", " ", " ", "x"])
        logic.collision_detector.handle_player_death(game_map, player2, self.projectile, "p2")
